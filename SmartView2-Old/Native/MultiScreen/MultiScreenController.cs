...
// Type: SmartView2.Native.MultiScreen.MultiScreenController
// Assembly: Smart View 2.0, Version=1.1.0.22852, Culture=neutral, PublicKeyToken=null
// MVID: E41C38D8-0DED-4C9C-BA2D-5421E4C1BA15
// Assembly location: C:\Program Files (x86)\SmartView2\Smart View 2.0.exe
...
using SmartView2.Native.MultiScreen.HttpHandler;
...
namespace SmartView2.Native.MultiScreen
{
    public class MultiScreenController : IMultiScreen, IDisposable, INotifyPropertyChanged
    {
        ...
        {
            Logger.Instance.LogMessageFormat("[SmartView2][MultiScreenController]Create MultiScreenController");
        ...
        {
            Logger.Instance.LogMessageFormat("[SmartView2][MultiScreenController]InitializeAsync started...");
            Logger.Instance.LogMessageFormat("[SmartView2][MultiScreenController]LoadState is {0}", (object)this.LoadState.ToString());
            if (this.LoadState == LoadState.Stoped || this.LoadState == LoadState.Failed)
            {
                ...
                this.httpServer.AddHandler((IHttpHandler)new FileHandler(this.shareFiles));
                this.httpServer.AddHandler((IHttpHandler)new CoverHandler(this.shareFiles));
            }
        }

        public void Close()
        {
            Logger.Instance.LogMessageFormat("[SmartView2][MultiScreenController]Close started...");
            Logger.Instance.LogMessageFormat("[SmartView2][MultiScreenController]LoadState is {0}", (object)this.LoadState.ToString());
        ...
        {
            Logger.Instance.LogMessageFormat("[SmartView2][MultiScreenController]PushMediaToTv started...");
        ...
        {
            Logger.Instance.LogMessageFormat("[SmartView2][MultiScreenController]PushMediaToTv PushMediaToTvQueue...");
            this.TryStartMultiscreenApp();
            if (this.LoadState == LoadState.Loading)
            {
                if (files == null)
        ...
        {
            Logger.Instance.LogMessageFormat("[SmartView2][MultiScreenController]deviceResultByDevice_OnSuccess started...");
            if (device == null)
                return;
            this.connectedDevice = device;
            Logger.Instance.LogMessageFormat("[SmartView2][MultiScreenController]deviceResultByDevice_OnSuccess connectedDevice GetApplication : {0}", (object)"Mediashare");
        ...
        {
            Logger.Instance.LogMessageFormat("[SmartView2][MultiScreenContorller]deviceResultByDevice_OnFailed with Code: {0}, message: {1}, ", (object)error.Code, (object)error.Message);
        ...
        {
            Logger.Instance.LogMessageFormat("[SmartView2][MultiScreenController]applicationResult_OnSuccess started...");
            if (application == null)
                return;
        ...
        {
            Logger.Instance.LogMessageFormat("[SmartView2][MultiScreenContorller]applicationResult_OnFailed with Code: {0}, message: {1}, ", (object)e.Code, (object)e.Message);
            this.LoadState = LoadState.Failed;
            this.OnMultiscreenStartFailed();
        }
        ...
        {
            Logger.Instance.LogMessageFormat("[SmartView2][MultiScreenController]boolResult_OnSuccess started...");
            Logger.Instance.LogMessageFormat("[SmartView2][MultiScreenController]boolResult_OnSuccess connectedDevice ConnectToChannel by id: {0}", (object)"multiscreen_media_share");
            await Task.Delay(2000);
            this.connectedDevice.ConnectToChannel("multiscreen_media_share", (DeviceAsyncResult<com.samsung.multiscreen.channel.Channel>)this.deviceResultByChannel);
        }

        private void boolResult_OnFailed(object sender, ApplicationError e)
        {
            Logger.Instance.LogMessageFormat("[SmartView2][MultiScreenContorller]boolResult_OnFailed with Code: {0}, message: {1}, ", (object)e.Code, (object)e.Message);
            this.LoadState = LoadState.Failed;
            this.OnMultiscreenStartFailed();
        }
        ...
        {
            Logger.Instance.LogMessageFormat("[SmartView2][MultiScreenController]deviceResultByChannel_OnSuccess started...");
            this.appChannel = channel;
            this.appChannel.Listener = (IChannelListener)this.channelListner;
            this.LoadState = LoadState.Loaded;
            foreach (Tuple<Content, bool> tuple in this.loadingQueue)
            {
                if (tuple.Item1 != null)
                {
                    if (tuple.Item2)
                        this.PushMediaToTvQueue(tuple.Item1);
                    else
                        this.PushMediaToTv(tuple.Item1);
                }
            }
            this.loadingQueue.Clear();
            this.SetSlideShowSettings(new SlideShowSettingsModel()
            {
        ...
        {
            Logger.Instance.LogMessageFormat("[SmartView2][MultiScreenContorller]deviceResultByChannel_OnFailed with Code: {0}, message: {1}, ", (object)e.Code, (object)e.Message);
            this.LoadState = LoadState.Failed;
            this.OnMultiscreenStartFailed();
        }
        ...
        private void channelListner_MessageReceived(object sender, string message)
        {
            ...
            switch (str)
            {
                ...
                case "mediaPanelPlayer.error":
                    Logger.Instance.LogMessageFormat("[SmartView2][MultiScreenController]channelListner_MessageReceived MESSAGE_TYPE_PANEL_ERROR: {0}", (object)message);
